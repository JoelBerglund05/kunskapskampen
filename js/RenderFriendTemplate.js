...
export default class RenderFriendTemplate extends RenderTemplate {
  ...
  async RenderFriendList(dataBase) {
    ...
    for (let i = 0; i < friends.friends.length; i++) {
      ...
      this.friendContainer.querySelectorAll("h2.p-2")[i].textContent =
        friends.friends[i].name;
      this.friendContainer
        .querySelectorAll(".play-against-btn")
        [i].setAttribute("id", "playAgainst" + i);
    }
  }
}
